...
class PolicyValueFunction:
    ...
    def __call__(self, qs: List[Tuple[str, str]]) -> List[List[Tuple[str, float]]]:
        ...
        Args:
            qs: List of (question, state) tuples
            
        Returns:
            List of lists of (next_state, value) tuples, one list per input question-state pair
        """
        if not qs: 
            return []
            
        try:
            # First, get policy predictions
            ...
            # Now get value predictions for all next states
            ...
            value_resp = requests.post(
                ...
                json={
                    "questions_and_states": value_inputs
                },
                headers={"Content-Type": "application/json"},
                timeout=60
            )
