...
def finetune_value(train_config):
    ...
    validation_data = convert_hf_data_to_value_dataset(train_config["validation_data_file"], "validation")
    
    #model = ValueFunction(train_config["model_name"]).to(train_config["device"])
    model = torch.load(train_config["model_name"], weights_only=False)
    ...
    for epoch in range(train_config["max_epochs"]):
        ...
        for step, batch in progress_bar:
            ...
            if num_batches % train_config["logging_steps"] == 0:
               print(running_loss)
            ...
            epoch_step = train_config["eval_steps"] #after what percentage of epoch we should evalaute
            ...
            if global_step % epoch_step == 0:
                ...
                if val_loss < best_loss and global_step % train_config["save_steps"] == 0:
                    best_loss = val_loss
                    patience_counter = 0
